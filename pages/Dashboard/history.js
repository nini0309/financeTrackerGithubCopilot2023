  ...
  const DOM = {
    ...
    innerHTMLTransaction(transaction, index) {
      ...
      const CSSclass = transaction.amount > 0 ? "income" : "expense";
        if(transaction.amount>0?"income":"expense"==false){
            incomes= Utils.formatCurrency(transaction.amount);
            expenses='-';
        }
        else{
            expenses= Utils.formatCurrency(transaction.amount);
            incomes='-';
...
const showIncomesButton = document.getElementById("showIncomesButton");
showIncomesButton.addEventListener("click", displayIncomes);

const showExpensesButton = document.getElementById("showExpensesButton");
showExpensesButton.addEventListener("click", displayExpenses);

const showAllButton = document.getElementById("showAllButton");
showAllButton.addEventListener("click", displayAll);

function displayAll(){
    const allTable = document.getElementById("data-table");
    const allTableBody = allTable.querySelector("tbody");

    // Clear the table body
    allTableBody.innerHTML = "";

    // Iterate through the transactions and display incomes
...
function displayIncomes() {
    const incomeTable = document.getElementById("data-table");
...
function displayExpenses() {
    const expenseTable = document.getElementById("data-table");
    const expenseTableBody = expenseTable.querySelector("tbody");

    // Clear the table body
    expenseTableBody.innerHTML = "";

    // Iterate through the transactions and display expenses
    Transaction.all.forEach((transaction, index) => {
        if (transaction.amount < 0) {
            const tr = document.createElement("tr");
            tr.innerHTML = DOM.innerHTMLTransaction(transaction, index);
